...
CxSkinButton::CxSkinButton()
{
	...
	m_colNomal = m_colDown = m_colOver = m_colDisable = GetSysColor(COLOR_BTNFACE);
...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (lpDrawItemStruct->itemState & ODS_DISABLED){	// DISABLED BUTTON
		if(m_bDisabled.m_hObject==NULL)
			...
			pDC->FillSolidRect(&r,m_colDisable);
		else { // paint the skin
			pDC->FillSolidRect(&r,m_colDisable);
		...
		if ((lpDrawItemStruct->itemState & ODS_SELECTED)||m_Checked){
#if 0	// by uhruh
			if (m_Checked && !(lpDrawItemStruct->itemState & ODS_SELECTED)){
				if(m_bOver.m_hObject==NULL)
					// no skin selected for selected state -> standard button
					pDC->FillSolidRect(&r,m_colOver);
				else { // paint the skin
					pDC->FillSolidRect(&r,m_colOver);
			...
			if(m_bNormal.m_hObject==NULL)
				...
				pDC->FillSolidRect(&r,m_colNomal);
			else // paint the skin
#if 0	// by uhruh
				if (!(GetButtonStyle() | BS_CHECKBOX) && (m_tracking)&&(m_bOver.m_hObject!=NULL)){
#else
...
//	DrawMode: 0=Normal; 1=stretch; 2=tiled fill
	...
	if (m_bMask.m_hObject!=NULL){
		...
		if(!DrawMode){
			...
			int bx=GetBitmapWidth(hbmp);
			int by=GetBitmapHeight(hbmp);
			INT nX = (INT)(((DOUBLE)rect.Width() / 2.0) - ((DOUBLE)bx / 2.0));
			INT nY = (INT)(((DOUBLE)rect.Height() / 2.0) - ((DOUBLE)by / 2.0));
			dc->BitBlt(nX /*r.left*/,nY/*r.top*/,cx,cy,&dcBmp,0,0,SRCCOPY);
		} else {
			int bx=GetBitmapWidth(hbmp);
			int by=GetBitmapHeight(hbmp);
...
/////////////////////////////////////////////////////////////////////////////
void CxSkinButton::SetBkgndColor(COLORREF nomal, COLORREF down, COLORREF over_sel, COLORREF disable)
{
	m_colNomal		= nomal;
	m_colDown		= down;
	m_colOver		= over_sel;
	m_colDisable	= disable;
}
//////////////////////////////////////////////////////////////////////////
void CxSkinButton::SizeToSkin()
{
	INT nWidth = 0, nHeight = 0;

	if(m_bNormal.GetSafeHandle())
		nWidth = GetBitmapWidth(m_bNormal), nHeight = GetBitmapHeight(m_bNormal);
	if(m_bDown.GetSafeHandle())
		nWidth = GetBitmapWidth(m_bDown), nHeight = GetBitmapHeight(m_bDown);
	if(m_bOver.GetSafeHandle())
		nWidth = GetBitmapWidth(m_bOver), nHeight = GetBitmapHeight(m_bOver);
	if(m_bFocus.GetSafeHandle())
		nWidth = GetBitmapWidth(m_bFocus), nHeight = GetBitmapHeight(m_bFocus);
	...
	if(nWidth && nHeight)
		...
		SetWindowPos(	NULL, -1, -1, nWidth, nHeight, SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOACTIVATE);
}
/////////////////////////////////////////////////////////////////////////////
...
//
void CxSkinButton::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CxSkinButton::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CxSkinButton::ActivateTooltip(BOOL bActivate)
{
...
//
DWORD CxSkinButton::EnableBalloonTooltip()
{
	m_dwToolTipStyle |= TTS_BALLOON;
	return BTNST_OK;
} // End of EnableBalloonTooltip

BOOL CxSkinButton::PreTranslateMessage(MSG* pMsg)
{
	InitToolTip();
