...
int CxStatic::GetFontPointSize(int nHeight)
{
	...
	int nPointSize = 24;//WCEMulDiv(nHeight, 72, cyPixelsPerInch);
	if (nPointSize < 0)
...
int CxStatic::GetFontHeight(int nPointSize)
{
	...
	int nHeight = 20;//-WCEMulDiv(nPointSize, cyPixelsPerInch, 72);
...
void CxStatic::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CStcMemDC MemDC(pDC, &rect, m_bTransparentBk);
...
void CxStatic::OnPaint()
{
	...
	CStcMemDC MemDC(&dc, &rc, m_bTransparentBk);
...
//	MAP_STYLE(	SS_PATHELLIPSIS,	DT_PATH_ELLIPSIS			);
	...
	if ( m_bAutoWrapping ){
		for (int i = 1; i <= csText.GetLength(); i++){
			...
			if (csText.GetAt(i-1) == _T('\r') || csText.GetAt(i-1) == _T('\n')){
				if (nCR == 0){
